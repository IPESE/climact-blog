...
To create a new blog post, simply copy the [\_template](https://github.com/IPESE/climact-blog/edit/main/climact-blog/_template/index.qmd) folder and paste it into the folder for the submenu where you want your post to be. For the moment only the `posts` directory is availbale for post on the "Home" menu.
...
1.  In the sidebar on the left, go to the git menu: ![git-menu](images/git-icon.png)
2.  Here you can find a list of files you have changed or added. Click on the plus icon next to changes to stage all files or add one by one.![stage](images/stage-icon.png)
3.  Write a short message that represents the changes you have made.
4.  Click on the arrow next to Commit and choose "Commit & Push" ![commit&push](images/commit&push-icon.png)